...
export function PdfDownloader({ reportId }: { reportId: number }) {
  ...
  const handleDownload = () => {
    ...
    const accordions = document.querySelectorAll<HTMLElement>(
      "[data-accordion-content], [data-calendar-body]"
    );
    ...
    setTimeout(() => {
      ...
      if (tg?.close && tg?.initData && tg.initData.length > 10) {
        ...
        toast.textContent =
          "✓ PDF tayyor. 10 sekunddan keyin botga qaytasiz...";
        toast.style.cssText = `
            ...
            <div className="p-4 space-y-2">
              {THEMES.map((t) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`w-11 h-11 rounded-lg ${t.bg} flex items-center justify-center shrink-0`}
                    >
                      <Icon className={`w-5 h-5 ${t.color}`} />
                    </div>
                    <div className="flex-1 text-left">
                      <p className="font-semibold text-sm">{t.label}</p>
                      <p className="text-[11px] text-[var(--tg-hint)]">
                        {t.desc}
                      </p>
                    </div>
                    {active && (
