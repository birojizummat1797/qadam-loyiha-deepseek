# -*- coding: utf-8 -*-
"""Qadam.io — Admin panel UI + Feedback modal."""
from pathlib import Path

# ═══════════════════════════════════════════════════════════
# 1. ADMIN PANEL SAHIFA
# ═══════════════════════════════════════════════════════════
ADMIN_DIR = Path("qadam-miniapp/app/admin")
ADMIN_DIR.mkdir(parents=True, exist_ok=True)

ADMIN_PAGE = ADMIN_DIR / "page.tsx"
...
''', encoding="utf-8")
print("[OK] app/admin/page.tsx")

# ═══════════════════════════════════════════════════════════
# 2. FEEDBACK MODAL
# ═══════════════════════════════════════════════════════════
FEEDBACK = Path("qadam-miniapp/components/FeedbackModal.tsx")
...
# ═══════════════════════════════════════════════════════════
# 3. REPORT PAGE — Feedback qo'shish
# ═══════════════════════════════════════════════════════════
REPORT = Path("qadam-miniapp/app/report/[id]/page.tsx")
rep = REPORT.read_text(encoding="utf-8")

if "FeedbackModal" not in rep:
    rep = rep.replace(
        'import { CloseButton } from "@/components/CloseButton";',
        'import { CloseButton } from "@/components/CloseButton";\nimport { FeedbackModal } from "@/components/FeedbackModal";',
    )
    # CloseButton'dan oldin FeedbackModal
    rep = rep.replace(
        "<CloseButton />",
        "<FeedbackModal reportId={Number(id)} />\n\n      <CloseButton />",
    )
    REPORT.write_text(rep, encoding="utf-8")
    print("[OK] report/page.tsx — FeedbackModal qoshildi")
else:
    print("[SKIP] report/page.tsx — allaqachon bor")

# ═══════════════════════════════════════════════════════════
# 4. BOT — /admin komandasi
# ═══════════════════════════════════════════════════════════
START = Path("qadam/bot/handlers/start.py")
start = START.read_text(encoding="utf-8")

if "/admin" not in start:
    start += '''
...
'''
    ...
    print("[OK] bot/handlers/start.py — /admin komanda")
else:
    print("[SKIP] bot/handlers/start.py — allaqachon bor")

print()
print("=" * 60)
print("Admin UI + Feedback — TAYYOR!")
print("=" * 60)
print()
print("KEYINGI:")
print("  1. git add -A")
print('  2. git commit -m "Admin UI + Feedback modal"')
print("  3. git push")
print("  4. Render Manual Deploy (backend uchun)")
print()
print("Keyin:")
print("  • Admin panel: WEBAPP_URL/admin")
print("  • Feedback: report sahifasida")